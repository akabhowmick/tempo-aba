...
export const SupportChat: React.FC = () => {
  ...
  return (
    <div>
      <button
        className="z-50 fixed bottom-4 right-4 inline-flex items-center justify-center text-md font-medium border rounded-full w-16 h-16 bg-gray-100 cursor-pointer border-green-800 p-0 hover:text-gray-900"
        type="button"
        onClick={toggleChat}
      >
      ...
      {isChatOpen && (
        ...
        >
          <div className="flex flex-col space-y-1.5 pb-6">
            ...
            <p className="text-md text-gray-600 leading-5">
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                name="name"
                required
                className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-md focus:ring-green-500 focus:border-green-500"
            ...
            {/* Topic/Subject of Inquiry */}
            <div>
              <label className="block text-md font-medium text-gray-700">Topic/Subject</label>
              <select
                name="topic"
                required
                value={selectedTopic} // Set the value to the controlled state variable (see below for changes)
                className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-md focus:ring-green-500 focus:border-green-500"
                onChange={(e) => setSelectedTopic(e.target.value)} // Add an onChange handler
                size={3}
              >
                <option value="" disabled>
                  Select a topic
                </option>
                <option className="mt-1"value="general">General question or inquiry</option>
                <option className="mt-1"value="services">I am interested in services for my child(ren)</option>
                <option className="mt-1"value="recruiting">Recruiting and hiring</option>
              </select>
            </div>
            ...
            <div className="text-center">
              <button
                type="submit"
                className="inline-flex items-center justify-center rounded-md text-md font-medium text-white bg-green-800 hover:bg-green-700 px-4 py-2"
              >
                Send Message
              </button>
            </div>
